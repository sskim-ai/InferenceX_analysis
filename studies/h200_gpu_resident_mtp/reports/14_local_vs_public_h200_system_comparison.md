...
>
> **중요:** InferX의 `HTTP P90=7`, prefill TP/CP rank envelope, decode DP-shard occupancy는 서로 다른 metric/scope다. Decode-stage reconstructed occupancy는 mean **5.14**, P90 **10**, P95 **12**, highest reconstructed **1-s-bin sum 17\***이지만, 이는 instantaneous unique-request maximum이 아니다. Prefill unique union과 P/D 전체 unique global running/waiting 총합은 여전히 **Unknown**이다.
...
| TTFT − queue mean proxy | **0.770 s** | **1.812 s** | observed TTFT 1.081 s; limited public queue metrics are low† |
...
† InferX queue/KV 숫자는 endpoint/rank-series scope이며 Local의 global metric과 동일 scope가 아니다. Generic decode queue가 0이어도 P/D-specific preallocation 및 transfer queue가 모두 0이라는 뜻은 아니다.
...
| Decode reconstructed occupancy mean | N/A | N/A | **5.14** | decode stage; exported 1-s scheduler-occupancy bins |
| Decode reconstructed occupancy P90 | N/A | N/A | **10** | decode stage; exported 1-s scheduler-occupancy bins |
| Decode reconstructed occupancy P95 | N/A | N/A | **12** | decode stage; exported 1-s scheduler-occupancy bins |
| Decode highest reconstructed 1-s-bin sum | N/A | N/A | **17\*** | instantaneous unique-request max가 아님 |
| InferX decode worker reference | N/A | N/A | validated decode workers max **9/10** | TP8/DP8 DP-shard sum; worker-stage scope |
| **Scheduler waiting — global/cluster total** | **max 1** | **max 25** | **unique P/D global Unknown** | Local은 global 관측. InferX generic decode queue만 0이며 P/D-specific queues는 별도 scope |
...
| Generic decode waiting queue max | N/A | N/A | **0** | `sglang:num_queue_reqs`만 해당 |
| Decode prealloc queue max / P90 | N/A | N/A | **5 / 2** | P/D-specific decode preallocation queue |
| Decode transfer queue max / P90 | N/A | N/A | **4 / 1** | P/D-specific decode transfer queue |
| InferX prefill unique running/waiting | N/A | N/A | **Unknown** | CP-rank request union unavailable |
...
\* `17`은 exported backend one-second scheduler-occupancy bins를 endpoint interval intersection으로 합친 최고값이다. instantaneous unique user-request count가 아니다.
...
+ InferX decode-stage highest reconstructed 1-s-bin sum = 17*
...
- 따라서 **InferX에서 P/D 전체의 unique request 총수는 현재 Unknown**으로 남겨야 한다. Decode-stage `17`은 exported 1초 occupancy bin의 최고 합계이며 global GPU service total 또는 instantaneous unique request total이 아니다.
...
| Cluster-global scheduler running | max 4 | max 7 | **P/D unique global Unknown**; decode occupancy mean 5.14 / P90 10 / P95 12 / highest 1-s-bin sum 17* |
| Cluster-global scheduler waiting | max 1 | max 25 | **P/D unique global Unknown**; generic decode queue 0, prealloc max/P90 5/2, transfer 4/1 |
| Queue regime | 거의 queue-free | deep saturation | generic decode queue=0이나 P/D-specific queues는 active; P/D global queue regime은 Unknown |
...
| Cluster-global scheduler running max | 4 | **7** | **P/D unique global Unknown**; decode highest reconstructed 1-s-bin sum **17\***, P90 **10** |
| Cluster-global scheduler waiting max | 1 | **25** | **P/D unique global Unknown**; generic decode queue **0**, prealloc max/P90 **5/2**, transfer **4/1** |
| InferX scheduler 참고값 | N/A | N/A | prefill rank-envelope running≤5/4, waiting≤5/5; decode workers max 9/10, occupancy mean 5.14 / P95 12 |
...
| Queue median | 0.012 s | **24.385 s** | prefill queue histogram median ≈2–3 ms† |
...
Local conc8의 높은 TTFT를 `max_num_seqs=8` 하나로 설명할 수는 없다. Local에서는 HTTP fan-out max 29에 대해 global running max 7, waiting max 25, KV usage P95 98%가 함께 관측됐다. 반면 InferX는 P/D-disaggregated 32-GPU 구조에서 decode-stage reconstructed occupancy가 exported 1-s bins 기준 mean 5.14 / P90 10 / P95 12였고, generic SGLang decode waiting은 0이었다. 다만 InferX의 decode preallocation/transfer queues는 active였고 unique P/D global running/waiting은 Unknown이다. 따라서 Local running=7과 InferX P90=10을 직접 성능비로 읽지 않으며, 이 수치는 두 시스템의 serving capacity와 resource topology가 근본적으로 다름을 보여주는 scope-labelled evidence로만 사용한다.
...
## 5.1 Local low-contention contextual baseline
...
Local conc1은 직접 관측된 low-contention baseline이다. InferX c8은 generic decode waiting이 0이고 짧은 공개 queue histogram 값이 보이지만, 별도 P/D preallocation/transfer queues가 active이며 P/D global queue regime은 Unknown이다. 따라서 두 시스템을 동일한 "queue-free" 상태로 확정하지 않고, observed service behavior의 제한된 맥락 비교로만 사용한다.
...
† InferX 1.081초는 queue-adjusted 값이 아니라 observed median TTFT다. Prefill queue histogram median은 약 2–3ms이고 generic decode queue는 0이지만, P/D-specific preallocation/transfer queues가 active였다. 이는 P/D global queue-time decomposition이나 global queue pressure 결론이 아니다.
...
| Weighted decode TPS | 93.05 | **61.71** | **90.45** |
| HTTP peak | 4 | **29** | **11** |
| Global scheduler running | max 4 | max 7 | **P/D unique global Unknown**; decode highest reconstructed 1-s-bin sum 17*, P90 10 |
| Global scheduler waiting | max 1 | max 25 | **P/D unique global Unknown**; generic decode queue 0; prealloc 5/2 and transfer 4/1 (max/P90) |
| Waiting regime | 거의 없음 | **persistent/deep saturation** | generic queue=0이지만 P/D-specific queues active; P/D global waiting regime Unknown |
...
4. **InferX의 P/D unique global scheduler running/waiting request 수는 Unknown이다.** `HTTP P90=7`은 running 7을 뜻하지 않는다. Decode DP-shard occupancy는 mean 5.14/P90 10/P95 12이고 highest reconstructed 1-s-bin sum은 17*이지만, 이는 instantaneous unique-request max가 아니다. Prefill CP-rank unique union 및 P/D handoff deduplication이 없어 global total로 합산하지 않는다.
5. Public generic decode `num_queue_reqs`는 0으로 관측됐지만, P/D-specific decode prealloc queue는 max/P90 5/2, transfer queue는 4/1이었다. Prefill unique queue union도 별도 scope다. 따라서 **global persistent waiting이 낮다**고 확정하지 않는다.
6. **Local conc8 raw TTFT가 InferX보다 매우 길지만 decode TPS 차이는 훨씬 작다.** 따라서 TTFT 격차의 상당 부분은 Local scheduler/admission backlog로 설명된다.
7. **Local conc1과 InferX는 decode TPS가 같은 order**다: 93.05 vs 90.45 tok/s. 이는 queue 상태 동일성이 증명된 비교가 아니라 observed service behavior의 제한된 맥락 비교이며 hardware efficiency 비교가 아니다.
...
> Local 2×H200 Aggregated 서버는 low-contention 상태에서는 빠른 service rate를 보이지만, 동일 ID03 workload에서 concurrency가 증가하면 scheduler/KV pressure가 빠르게 포화된다. Local conc8의 매우 큰 TTFT는 active inference 성능 저하만으로 설명되지 않으며 persistent queue backlog가 지배적이다. Public InferenceX c8은 동일 root-concurrency label을 사용하지만 실제 request-plane load가 훨씬 낮다. Public export에서는 P/D unique global running/waiting request 총수를 복원할 수 없으므로 Local의 global 7/25와 숫자 대 숫자로 비교하면 안 된다. Public decode-stage reconstructed occupancy는 exported 1초 bin 기준 mean 5.14/P90 10/P95 12/highest 17*이지만, prefill unique union 및 cross-stage deduplication은 Unknown이다.
...
- `InferX prefill rank-envelope 5/4 + decode highest reconstructed 1-s-bin sum 17 = global running`
- `InferX prefill TP/CP rank-envelope waiting max 5 = cluster-global waiting 5`
- `generic decode queue=0이므로 모든 decode/P/D queue가 0이다`
- `decode highest reconstructed 1-s-bin sum 17은 instantaneous unique request max다`
- `2 GPU가 32 GPU보다 빠르다`
...
studies/h200_gpu_resident_mtp/processed/c8_cluster_scheduler_reconstruction.csv
studies/h200_gpu_resident_mtp/processed/c8_decode_timeslice_field_semantics.csv
studies/h200_gpu_resident_mtp/processed/c8_decode_cluster_field_sensitivity.csv
studies/h200_gpu_resident_mtp/processed/c8_decode_timeslice_alignment_summary.csv
