...
.PHONY: bootstrap preflight acquire-run acquire-traces inspect build-source build-h200 join coverage analyze-ids analyze-concurrency validate figures report test lint secret-scan all mtp-acquire mtp-analyze mtp-id03 mtp-c8-concurrency mtp-c8-cluster-scheduler mtp-report mtp-all
...
mtp-c8-concurrency:
	mkdir -p .mplconfig
	MPLCONFIGDIR=$(CURDIR)/.mplconfig $(PYTHON) scripts/h200_gpu_resident_mtp/reconstruct_c8_concurrency.py

mtp-c8-cluster-scheduler:
	mkdir -p .mplconfig
	MPLCONFIGDIR=$(CURDIR)/.mplconfig $(PYTHON) scripts/h200_gpu_resident_mtp/reconstruct_c8_scheduler_cluster.py

mtp-report:
	mkdir -p .mplconfig
	MPLCONFIGDIR=$(CURDIR)/.mplconfig $(PYTHON) scripts/h200_gpu_resident_mtp/build_reports.py

mtp-all: mtp-analyze mtp-id03 mtp-c8-concurrency mtp-c8-cluster-scheduler mtp-report
